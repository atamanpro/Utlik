...
DEFAULT_PROMPT = '''You are an assistant for question-answering tasks. Use the following pieces of retrieved context to answer the question.
...
if os.environ.get('CUSTOM_OPENAI_BASE_URL') != 'empty':
    llm = ChatOpenAI(model=MODEL_NAME, temperature=temperature, api_key=OPENAI_API_KEY, base_url=os.environ.get('CUSTOM_OPENAI_BASE_URL'))
...
DOCS = load_documents('local', S3_DATA_BUCKET, ['txt', 'json', 'docx'])
